...
public interface ShopRepository extends JpaRepository<Shop,Integer>{
	...
	@Query(value = "Select shop_name,description from Shop where lower(shop_name) like lower(concat('%',:shopName, '%'))", nativeQuery = true)
	List<ShopProjection> findAllByShopName(String shopName);
	
	Optional<Shop> findByShopName(String shopName);
	
	@Query(value = "Select shop_name,description from Shop where shop_id = :shopId", nativeQuery = true )
	ShopProjection findShopById(int shopId);
